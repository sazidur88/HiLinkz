<div class="works rn-column-area rn-section-gap-work bg_color--5" id="works">
    <div class="container">
        <div class="row">
            <div class="col-lg-6">
                <div class="section-title text-left mb--20 mb_sm--0">
                    <h2 class="title">Our Works</h2>
                    <p>HiLinkz partners for exponential growth through innovative technology solutions. As a top consulting and app development agency we have a decade of experience in building innovative technology solutions for our clients.</p>

                </div>
            </div>


            <!-- ======= Works Section ======= -->

            <div class="col-lg-12">

                <div class="row mt--30">
                    <a href="{{route('our_works_ryogas')}}">
                        <div class="col-lg-4 col-md-6 sm-12 d-flex align-items-stretch mt--30" data-aos="fade-up" data-aos-delay="200">
                            <div class="card" style="background-image: url({{asset('assets/images/work/portfolio-2.jpg')}});">

                                <div class="card-body">
                                    <h5 class="card-title gradient-work">RyoGas</h5>
                                    <!-- <p class="card-text">Our Vision is to make the LPG, CNG, Or any Other Fuel stations into Next Generation Smart Stations.</p> -->
                                    <p class="card-text">
                                        A Smart Gas Pump Solution<br> <br>

                                    </p>
                                    <div class="read-more pt--10"><a href="{{route('our_works_ryogas')}}">Read more <i class="fas fa-arrow-right"></i></a></div>
                                </div>
                            </div>
                        </div>
                    </a>
                    <a href="{{route('dipspace')}}">
                        <div class="col-lg-4 col-md-6 sm-12 d-flex align-items-stretch mt--30" data-aos="fade-up">
                            <div class="card" style="background-image: url({{asset('assets/images/work/DipSpace.jpg')}});">
                                <div class="card-body">
                                    <h5 class="card-title gradient-work">DipSpace</h5>
                                    <p class="card-text">
                                        To dip into digital space for digital asset management.
                                    </p>
                                    <div class="read-more pt--10"><a href="{{route('dipspace')}}">Read more <i class="fas fa-arrow-right"></i></a></div>
                                </div>
                            </div>
                        </div>
                    </a>
                    <a href="{{route('our_works_covi_sense')}}">
                        <div class="col-lg-4 col-md-6 sm-12 d-flex align-items-stretch mt--30" data-aos="fade-up" data-aos-delay="100">
                            <div class="card" style="background-image: url({{asset('assets/images/work/CoviSense.jpg')}});">
                                <div class="card-body">
                                    <h5 class="card-title gradient-work">CoviSense </h5>
                                    <p class="card-text">
                                        A low cost IoT device for sensing COVID Symptoms
                                    </p>
                                    <div class="read-more pt--10"><a href="{{route('our_works_covi_sense')}}">Read more <i class="fas fa-arrow-right"></i></a></div>
                                </div>
                            </div>
                        </div>
                    </a>
                </div>
            </div>
        </div>
        <div class="button-group mt--50 text-center btn-work">
            <a class="btn-transparent rn-btn-dark" href="{{route('our_works')}}"><span class="text font-italic">View More Works </span><i class="fas fa-arrow-right"></i></a>
        </div>
    </div>
</div>